...
### Requirement: Full meta line revealed inline on hover (no tooltip)

The municipio indicadores list SHALL reveal the full meta context string `` `<metaCodigo> · <metaNombre>` `` (same characters, separator, and ordering as the legacy single-line subtitle) **inline** in the name column when the user hovers the pointer over that subtitle area, so the line **looks the same as the pre-change subtitle** while hovered.
...
- **WHEN** the user hovers the pointer over the meta subtitle (compact state) for a row that has both `metaCodigo` and `metaNombre`
...
- **WHEN** the user hovers the pointer over the meta subtitle
