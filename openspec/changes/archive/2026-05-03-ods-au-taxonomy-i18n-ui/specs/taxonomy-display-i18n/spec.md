...
For each objective icon `<img>` in `OdsSelector`, the `alt` attribute SHALL be set to the exact same localized objective title string used for that objective’s `t(\`ods_${n}_name\`)` or `t(\`au_${n}_name\`)` in the same render (same `n` and taxonomy). The SHALL NOT use hardcoded literals such as `ODS ${n}` or `OE ${n}` as the sole or primary alternative text.
