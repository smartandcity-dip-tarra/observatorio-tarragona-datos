...
Normative requirements for English (`en`) as a first-class UI language in the Nuxt viewer: `@nuxtjs/i18n` registration and `en.json` messages, stable `/en` URL prefix where applicable, and SQLite-backed labels from `DICCIONARIO_EN` / `METADATA_EN` when APIs receive `lang=en`, with fallbacks to Catalan and Spanish when English rows are missing.
